@extends('web_new.layouts.main')

@section('content')
    <div class="main-section ">
        <div class="container">

            <div class="row ps-border-bottom mx-4">
                <div class="col-12 p-0">
                    <h1 class="heading-xl cl-lBlue fw-500">Features</h1>
                    <p class="paragraph cl-dblue">
                        See over run your practice with beautifully designed ways to work.
                    </p>
                </div>
            </div>

            <!-- Card deck -->
            <div class=" row mt-5">

                <div class="col-xs-12 col-sm-6 col-md-4">
                    <!-- Card -->
                    <a href="{{ url('feature/deatils') }}">
                        <div class="card">

                            <!--Card image-->
                            <div class="view overlay">
                                <img class="card-img-top"
                                    src="https://watermark.lovepik.com/photo/40007/3397.jpg_wh1200.jpg"
                                    alt="Card image cap">
                            </div>

                            <!--Card content-->
                            <div class="card-body">

                                <!--Title-->
                                <h4 class="card-title">Paperless Patient Management</h4>
                                <!--Text-->
                                <p class="card-text px-lext-lim">Some quick example text to build on the card title and
                                    make
                                    up the bulk of the
                                    card's content. Some quick example text to build on the card title and make up the
                                    bulk
                                    of the
                                    card's content</p>
                                <!-- Provides extra visual weight and identifies the primary action in a set of buttons -->
                                <p class="ps-read-more-btn m-0">Read more</p>

                            </div>
                    </a>

                </div>
                <!-- Card -->
            </div>

            <div class="col-xs-12 col-sm-6 col-md-4">
                <!-- Card -->
                <div class="card mb-4">

                    <!--Card image-->
                    <div class="card-img">
                        <img class="card-img-top"
                            src="https://img.freepik.com/free-photo/medical-banner-with-doctor-wearing-equipment_23-2149611201.jpg"
                            alt="Card image cap">
                    </div>

                    <!--Card content-->
                    <div class="card-body">

                        <!--Title-->
                        <h4 class="card-title">Advanced Performance Dashboard</h4>
                        <!--Text-->
                        <p class="card-text px-lext-lim">Some quick example text to build on the card title and make
                            up the bulk of the
                            card's content.</p>
                        <!-- Provides extra visual weight and identifies the primary action in a set of buttons -->
                        <a href="{{ url('/feature/deatils') }}" class="ps-read-more-btn">Read more</a>

                    </div>

                </div>
                <!-- Card -->
            </div>

            <div class="col-xs-12 col-sm-6 col-md-4">
                <!-- Card -->
                <div class="card mb-4">

                    <!--Card image-->
                    <div class="card-img">
                        <img class="card-img-top"
                            src="https://img.freepik.com/free-photo/close-up-people-wearing-lab-coats_23-2149126948.jpg?semt=ais_hybrid"
                            alt="Card image cap">
                    </div>

                    <!--Card content-->
                    <div class="card-body">

                        <!--Title-->
                        <h4 class="card-title">Custom Exercise Creation</h4>
                        <!--Text-->
                        <p class="card-text px-lext-lim">Some quick example text to build on the card title and make
                            up the bulk of the
                            card's content.</p>
                        <!-- Provides extra visual weight and identifies the primary action in a set of buttons -->
                        <a href="{{ url('/feature/deatils') }}" class="ps-read-more-btn">Read more</a>

                    </div>

                </div>
                <!-- Card -->
            </div>

            <div class="col-xs-12 col-sm-6 col-md-4">
                <!-- Card -->
                <div class="card">

                    <!--Card image-->
                    <div class="card-img">
                        <img class="card-img-top"
                            src="https://img.freepik.com/free-photo/hologram-feminine-silhouette-man-hand_23-2148755140.jpg?semt=ais_hybrid"
                            alt="Card image cap">
                    </div>

                    <!--Card content-->
                    <div class="card-body">

                        <!--Title-->
                        <h4 class="card-title">Boost Your Online Presence</h4>
                        <!--Text-->
                        <p class="card-text px-lext-lim">Some quick example text to build on the card title and make
                            up the bulk of the
                            card's content.</p>
                        <!-- Provides extra visual weight and identifies the primary action in a set of buttons -->
                        <a href="{{ url('/feature/deatils') }}" class="ps-read-more-btn">Read more</a>

                    </div>

                </div>
                <!-- Card -->
            </div>

            <div class="col-xs-12 col-sm-6 col-md-4">
                <!-- Card -->
                <div class="card mb-4">

                    <!--Card image-->
                    <div class="card-img">
                        <img class="card-img-top"
                            src="https://img.freepik.com/premium-photo/doctor-using-digital-tablet-against-digitally-generated-data-room_1134-23031.jpg?semt=ais_hybrid"
                            alt="Card image cap">
                    </div>

                    <!--Card content-->
                    <div class="card-body">

                        <!--Title-->
                        <h4 class="card-title">Connect with Your Existing Website</h4>
                        <!--Text-->
                        <p class="card-text px-lext-lim">Some quick example text to build on the card title and make
                            up the bulk of the
                            card's content.</p>
                        <!-- Provides extra visual weight and identifies the primary action in a set of buttons -->
                        <a href="{{ url('/feature/deatils') }}" class="ps-read-more-btn">Read more</a>

                    </div>

                </div>
                <!-- Card -->
            </div>

            <div class="col-xs-12 col-sm-6 col-md-4">
                <!-- Card -->
                <div class="card mb-4">

                    <!--Card image-->
                    <div class="card-img">
                        <img class="card-img-top"
                            src="https://img.freepik.com/free-photo/medical-banner-with-doctor-wearing-vr-glasses_23-2149611207.jpg?semt=ais_hybrid"
                            alt="Card image cap">
                    </div>

                    <!--Card content-->
                    <div class="card-body">

                        <!--Title-->
                        <h4 class="card-title">Track Patient Health Progress</h4>
                        <!--Text-->
                        <p class="card-text px-lext-lim">Some quick example text to build on the card title and make
                            up the bulk of the
                            card's content.</p>
                        <!-- Provides extra visual weight and identifies the primary action in a set of buttons -->
                        <a href="{{ url('/feature/deatils') }}" class="ps-read-more-btn">Read more</a>

                    </div>

                </div>
                <!-- Card -->
            </div>

            <div class="col-xs-12 col-sm-6 col-md-4">
                <!-- Card -->
                <div class="card mb-4">

                    <!--Card image-->
                    <div class="card-img">
                        <img class="card-img-top"
                            src="https://img.freepik.com/free-photo/medical-banner-with-doctor-wearing-equipment_23-2149611201.jpg"
                            alt="Card image cap">
                    </div>

                    <!--Card content-->
                    <div class="card-body">

                        <!--Title-->
                        <h4 class="card-title">Gather Feedback on Your Treatment</h4>
                        <!--Text-->
                        <p class="card-text px-lext-lim">Some quick example text to build on the card title and make
                            up the bulk of the
                            card's content.</p>
                        <!-- Provides extra visual weight and identifies the primary action in a set of buttons -->
                        <a href="{{ url('/feature/deatils') }}" class="ps-read-more-btn">Read more</a>

                    </div>

                </div>
                <!-- Card -->
            </div>

            <div class="col-xs-12 col-sm-6 col-md-4">
                <!-- Card -->
                <div class="card mb-4">

                    <!--Card image-->
                    <div class="card-img">
                        <!-- <img class="card-img-top"
                            src="https://img.freepik.com/free-photo/close-up-people-wearing-lab-coats_23-2149126948.jpg?semt=ais_hybrid"
                            alt="Card image cap"> -->
                            <i class="fas fa-link ps-card-icon"></i>
                    </div>

                    <!--Card content--> 
                    <div class="card-body">

                        <!--Title-->
                        <h4 class="card-title">Get Automated Suggestions</h4>
                        <!--Text-->
                        <p class="card-text px-lext-lim">Some quick example text to build on the card title and make
                            up the bulk of the
                            card's content.</p>
                        <!-- Provides extra visual weight and identifies the primary action in a set of buttons -->
                        <a href="{{ url('/feature/deatils') }}" class="ps-read-more-btn">Read more</a>

                    </div>

                </div>
                <!-- Card -->
            </div>

            <div class="col-xs-12 col-sm-6 col-md-4">
                <div class="wrapper card mb-4" style="height: 400px;">
                    <div class="card-loader card-loader--tabs"></div>
                    <div class="card-loader card-loader--tabs m-2" style="height: 30px;"></div>
                    <div class="card-loader card-loader--tabs m-2" style="height: 50px;"></div>
                    <div class="card-loader card-loader--tabs mb-2 ml-2 mt-4 mr-5" style="height: 30px;"></div>
                </div>
            </div>


        </div>
        <!-- Card deck -->

    </div>
    </div>

    <section class="testimonial-section mt-4">
        <div class="container">
            <blockquote class="m-0 cl-white">
                "Your Exercises Family, you guys are really awesome. Keep the amazing new features coming."
                <footer>
                    - Auto Solutions Best Practice Summit 2020
                </footer>
            </blockquote>
        </div>
    </section>


    <div class="container my-5">
        <div class="row pt-5 align-items-center justify-content-center">
            <div class="col-md-4 text-start mb-3">
                <h3 class="paragraph-lg cl-dblue">More than helpful software ðŸ’™ Join our Community!</h3>
                <p class="paragraph-md cl-dblue">As big as the world of health and wellness is, so is our community. Our
                    community is open to practitioners, clinic
                    owners, admin staff, students, and consultants in every industry. Join to ask questions, share
                    resources, and learn a
                    few new Your Exercises tips and tricks.</p>
                <a class="ps-btn primary-btn md-btn" href="book_a_demo">Learn More</a>
            </div>

            <div class="col-md-4 text-center mb-5">
                <img alt="Take a peek at the future of (even smoother) clinic life." class="img-fluid"
                    src="https://jane.app/assets/features/cover/janepayments-47a91655b28744ef9d858e407ad51d5f46dd6451c0123240a0abf91b80811a3c.png">
            </div>
        </div>
    </div>
@endsection