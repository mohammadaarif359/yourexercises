	...
	<div class="row">
	  <div class="col-md-12">
		<div class="card card-primary">
		  ...
		  <form role="form" id="frm-plan-submit" method="POST" action="{{ route('admin.plan.update') }}" enctype="multipart/form-data">
			@csrf
			...
			<div class="card-body">
				...
				<div id="itembody">
                @if($data['plan_detail'] && count($data['plan_detail']) > 0)
                @foreach($data['plan_detail'] as $count=> $deatil)
					...
					<div class="col-md-1">
						<div class="form-group">
							...
							<select id="reps{{$count}}" class="reps form-control @error('reps') is-invalid @enderror" name="detail[reps][]">
								...
								@for($i=1;$i<=50;$i++)
									<option value="{{ $i }}" {{ old('reps',$i == $deatil->reps ? 'selected' : '') }}>{{ $i }}</option>
								@endfor
							</select>
						...
						<div class="form-group">
							...
							<select id="hold{{$count}}" class="hold form-control @error('hold') is-invalid @enderror" name="detail[hold][]">
								...
								@foreach(config('custom.hold') as $k=> $val)
									<option value="{{ $k }}" {{ old('hold',$k == $deatil->hold ? 'selected' : '') }}>{{ $val }}</option>
								@endforeach
							</select>
					...
					<div class="col-md-1">
						<div class="form-group">
							...
							<select id="complete{{$count}}" class="complete form-control @error('complete') is-invalid @enderror" name="detail[complete][]">
								<option value='' selected>Select</option>
								@for($i=1;$i<=20;$i++)
									<option value="{{ $i }}" {{ old('complete',$i == $deatil->complete ? 'selected' : '') }}>{{ $i }}</option>
								@endfor
							</select>
					...
					<div class="col-md-1">
						<div class="form-group">
							...
							<select id="perform{{$count}}" class="perform form-control @error('perform') is-invalid @enderror" name="detail[perform][]">
								<option value='' selected>Select</option>
								@for($i=1;$i<=20;$i++)
									<option value="{{ $i }}" {{ old('perform', $i == $deatil->perform ? 'selected' : '') }}>{{ $i }}</option>
								@endfor
							</select>
						...
						<div class="form-group">
							...
							<select id="times{{$count}}" class="times form-control @error('times') is-invalid @enderror" name="detail[times][]" required>
								...
								@foreach(config('custom.times') as $k=> $val)
									<option value="{{ $k }}" {{ old('times', $k == $deatil->times ? 'selected' : '') }}>{{ $val }}</option>
								@endforeach
							</select>
			...
			<div class="card-footer">
				...
				<button type="button" class="btn btn-primary" id='btn-plan-submit'>Update</button>
			</div>
		</form>
...
@section('pagejs')
	...
	<script>
		var totalcount = {{ count($data['plan_detail']) > 0 ? count($data['plan_detail']) - 1 : 0  }};
		var planDetail = @json($data['plan_detail']);
